...
// eslint-disable-next-line @typescript-eslint/no-extraneous-class -- Service namespace pattern for API organization
class ReportsService {
  ...
  static async getInvestigatorWorkloadData(
    dateRange?: string,
  ...
  static async getTaskCompletionData(dateRange?: string): Promise<TaskCompletionData> {
    try {
  ...
  static async generateFraudReport(
  ...
  static async getAuditLogsData(dateRange?: string): Promise<AuditLogsData> {
    try {
  ...
  static async getCaseAgeingData(dateRange?: string): Promise<CaseAgeingData> {
    try {
  ...
  static async getEvidenceFindingsData(
    _dateRange?: string,
    ...
    try {
      ...
      const cases = Array.isArray(casesResponse)
        ? casesResponse
        : (casesResponse.data as Array<Record<string, unknown>> | undefined) ??
          (casesResponse.cases as Array<Record<string, unknown>> | undefined) ??
          [];
      ...
      for (const caseItem of cases) {
        ...
        try {
          // eslint-disable-next-line no-await-in-loop -- Sequential API calls needed for per-case evidence fetching
          ...
          if ('evidence' in caseEvidenceResponse) {
            ...
            if (Array.isArray(evidence)) {
              caseEvidence = evidence as Array<Record<string, unknown>>;
            }
      ...
      const err = ReportsService.handleError(error, 'fetch evidence findings data');
      console.error(
