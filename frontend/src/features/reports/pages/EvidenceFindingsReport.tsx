...
import { ExclamationCircleIcon, FunnelIcon, MagnifyingGlassIcon, DocumentIcon, EyeIcon, ArrowDownTrayIcon, XMarkIcon } from '@heroicons/react/24/outline';
...
const EvidenceFindingsReport: React.FC<EvidenceFindingsReportProps> = ({ dateRange = 'last30' }) => {
  const { data: evidenceData, isLoading, error } = useEvidenceFindings(dateRange);
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState<'All' | 'Confirmed' | 'Refuted' | 'Inconclusive'>('All');
  const [expandedId, setExpandedId] = useState<string | null>(null);
  const [modalState, setModalState] = useState<{ isOpen: boolean; filename: string; description: string }>({ isOpen: false, filename: '', description: '' });
  ...
  const handleExportPDF = async () => {
    try {
      ...
      await exportToPDF(formattedData, filename, 'Evidence Findings Report', columns);
    } catch (err) {
      console.error('Export failed:', err);
      alert('Export failed. Please try again.');
    }
  ...
  const handleViewEvidence = (filename: string) => {
    try {
      setModalState({ isOpen: true, filename, description: 'Transaction logs showing duplicate payments' });
    } catch (err) {
  ...
  const filteredFindings = findings.filter(finding => {
    ...
    const matchesStatus = statusFilter === 'All' || finding.conclusion === statusFilter;
    ...
    return (
      <div className="min-h-screen bg-gray-50">
        <div className="mx-auto px-4 sm:px-6 lg:px-8 max-w-screen-2xl">
          <div className="py-6">
            <h1 className="text-3xl font-bold text-gray-900">Evidence Findings Report</h1>
            <p className="mt-2 text-gray-600">Comprehensive view of all evidence items linked to investigation findings and conclusions</p>
          </div>
          <div className="pb-6">
          ...
          <div className="pb-6">
            <div className="bg-red-50 border border-red-200 rounded-lg p-6">
              <div className="flex items-center">
                ...
                <p className="text-red-700">Failed to load evidence findings data. Please try again.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="mx-auto px-4 sm:px-6 lg:px-8 max-w-screen-2xl">
        ...
        <div className="px-8 py-5 border-b border-gray-200">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-lg font-semibold text-gray-900">Investigation Findings</h3>
            </div>
        ...
        <div className="px-8 py-3 bg-gray-50 border-b border-gray-200">
          <p className="text-sm text-gray-600">
            Showing <span className="font-semibold">{paginatedFindings.length}</span> of <span className="font-semibold">{filteredFindings.length}</span> findings 
        ...
        <div className="divide-y divide-gray-200">
          {paginatedFindings.length > 0 ? (
            paginatedFindings.map((finding) => {
              const uniqueId = getUniqueFindingId(finding.caseId, finding.finding);
              const isExpanded = expandedId === uniqueId;
              return (
              <div key={uniqueId} className="p-8 hover:bg-gray-50/50 transition-colors">
                {/* Finding Header */}
                <div
                ...
                >
                  <div className="flex-1">
                    <div className="flex items-start gap-3 mb-2">
                      <div className="flex-1">
                        <h4 className="font-medium text-gray-900 group-hover:text-blue-600 transition-colors">{finding.finding}</h4>
                        <p className="text-xs text-gray-500 mt-1">
                          Case: <span className="font-mono">{finding.caseId}</span> | Date:{' '}
                    ...
                    <div className="flex items-center gap-2">
                      <span className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-medium ${getStatusColor(finding.conclusion)}`}>
                        {finding.conclusion}
                      </span>
                ...
                {isExpanded && (
                  <div className="mt-4 pt-4 border-t border-gray-200 space-y-4">
                    <div className="text-sm text-gray-600 mb-4">
                      <p><strong>Conclusion:</strong> {finding.conclusion}</p>
                    </div>
                    <div>
                      <h5 className="text-sm font-semibold text-gray-900 mb-3">Supporting Evidence</h5>
                      <div className="space-y-3">
                        {finding.supportingEvidence.map((evidence, idx) => (
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="font-medium text-gray-900 text-sm break-words">{evidence}</p>
                              <p className="text-xs text-gray-500 mt-1">2.40 KB · Jan 15, 2024, 09:30 AM</p>
                              <p className="text-xs text-gray-600 italic mt-1">Transaction logs showing duplicate payments</p>
                            </div>
            ...
            <div className="px-8 py-8 text-center">
              <p className="text-sm text-gray-500">No findings match your search criteria</p>
            </div>
          )}
        </div>
