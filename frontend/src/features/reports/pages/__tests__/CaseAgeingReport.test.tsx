...
vi.mock('../../components/ResolutionTimeTrendChart', () => ({
  default: ({ data, title }: any) => (
    <div data-testid="resolution-time-trend-chart">{title}</div>
  ),
}));

vi.mock('../../components/CaseAgeingPieChart', () => ({
  default: ({ data, title }: any) => (
    <div data-testid="case-ageing-pie-chart">{title}</div>
  ),
}));

vi.mock('../../components/CaseTypeResolutionChart', () => ({
  default: ({ data, title }: any) => (
    <div data-testid="case-type-resolution-chart">{title}</div>
  ),
}));
...
describe('CaseAgeingReport', () => {
  ...
  it('renders case ageing report with data', async () => {
    render(<CaseAgeingReport dateRange="last30" />, {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByTestId('resolution-time-trend-chart'),
      ).toBeInTheDocument();
      expect(screen.getByTestId('case-ageing-pie-chart')).toBeInTheDocument();
      expect(
        screen.getByTestId('case-type-resolution-chart'),
      ).toBeInTheDocument();
  ...
  it('renders loading state', () => {
    ...
    expect(
      screen.queryByTestId('case-ageing-stats-cards'),
  ...
  it('renders error state', () => {
    ...
    expect(
      screen.getByText(/Failed to load case ageing data/i),
    ).toBeInTheDocument();
  });
  ...
  it('passes correct dateRange to useCaseAgeing', () => {
    render(<CaseAgeingReport dateRange="last7" />, {
      wrapper: createWrapper(),
    });
