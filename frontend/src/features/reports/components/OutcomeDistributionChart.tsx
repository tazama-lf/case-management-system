...
const OutcomeDistributionChart: React.FC<OutcomeDistributionChartProps> = ({ data, title, height = 200 }) => {
  const maxValue = Math.max(...data.map(item => item.confirmed + item.refuted + item.inconclusive));
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
      ...
      <div className="space-y-4" style={{ height }}>
        {data.map((item, index) => {
          ...
          const confirmedWidth = (item.confirmed / total) * 100;
          const refutedWidth = (item.refuted / total) * 100;
          const inconclusiveWidth = (item.inconclusive / total) * 100;
