...
export class ProfileService {
  ...
  async generateProfile(alertId: number): Promise<GenerateProfileResponse> {
    if (!alertId) {
      return await Promise.reject(
        new Error('Alert ID is required to generate profile'),
      );
    }
  ...
  private handleError(error: unknown, operation: string): Error {
    if (error && typeof error === 'object' && 'response' in error) {
      ...
      if (errorWithResponse.response?.data) {
        return new Error(
          errorWithResponse.response.data.message ?? `Failed to ${operation}`,
        );
      }
      return new Error(
        `Failed to ${operation}: ${errorWithResponse.message ?? 'Unknown error'}`,
      );
    }
