...
export class TaskService {
  ...
  async reassignTask(
    ...
    try {
      const response = await apiClient.patch<{
        success: boolean;
        message: string;
      }>(`${this.baseUrl}/${taskId}/reassign`, { assignedUserId });
  ...
  async getWorkQueue(filters?: WorkQueueFilters): Promise<WorkQueueResponse> {
    try {
      ...
      const response = await apiClient.get<WorkQueueResponse>(
        `${this.baseUrl}/work-queue?${params}`,
      );
      return response;
    } catch (error: any) {
  ...
  async startTask(taskId: string): Promise<TaskForSupervisor> {
    return this.updateTaskForSupervisor(taskId, {
