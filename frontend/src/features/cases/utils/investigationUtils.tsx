...
export const loadEvidence = async (
    ...
    const getDisplayLabel = (type: string): string => {
        switch (type) {
            case 'KYC': return 'KYC/EDD Report';
            case 'SANCTIONS': return 'Sanctions Screening';
            case 'ADVERSE_MEDIA': return 'Adverse Media Screening';
            case 'SAR_STR_FILING': return 'SAR/STR Filing Documentation';
            case 'OTHER': return 'Other supporting Documentation and Reference Materials';
            default: return type;
        }
    };

    const ORDERED_TYPES = ['KYC', 'SANCTIONS', 'ADVERSE_MEDIA', 'SAR_STR_FILING', 'OTHER'];
