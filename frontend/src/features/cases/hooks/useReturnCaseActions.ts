...
export const useReturnCaseActions = (refreshCases: () => Promise<void>) => {
  ...
  const handleReturnForReview = async (
    ...
    try {
      ...
      const returnedCase = await caseService.returnCaseForReview(
        caseId,
        returnCaseData,
      );
      ...
      success(
        ...
        `Case ${caseId} has been sent back for additional review.
...
The investigator will receive your feedback and can make the necessary adjustments.`,
      ...
      let errorMessage =
        'Something went wrong while returning the case for review. Please try again.';
