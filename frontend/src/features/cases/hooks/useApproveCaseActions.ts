  ...
  const handleApproveClosureSubmit = async (
    ...
    try {
      ...
      success(
        ...
        `Perfect! You've approved the closure of case ${caseId}.
...
The case is now officially closed and removed from pending approvals.`,
      ...
      let errorMessage =
        'Something went wrong while approving the case closure. Please try again.';
  ...
  const handleApproveCreation = async (caseId: string) => {
    try {
      ...
      success(
        ...
        `Excellent! Case ${caseId} has been approved and is ready to go.
...
The case is now active and has been assigned to an investigator.`,
      ...
      let errorMessage =
        'Something went wrong while approving the case creation. Please try again.';
      const backendError = err instanceof Error ? err.message : '';
  ...
  const handleApproveReopening = async (caseId: string) => {
    try {
      ...
      success(
        ...
        `Great! Case ${caseId} has been approved for reopening.
...
The case is now back in the investigation queue.`,
      ...
      let errorMessage =
        'Something went wrong while approving the case reopening. Please try again.';
      const backendError = err instanceof Error ? err.message : '';
