...
describe('useEntityMetadata', () => {
  ...
  it('returns entity metadata on successful fetch', async () => {
    ...
    const { result } = renderHook(() => useEntityMetadata(101, 'tenant-1'), {
  ...
  it('does not fetch when alertId is 0', () => {
    const { result } = renderHook(() => useEntityMetadata(0, 'tenant-1'), {
      wrapper: createWrapper(),
    });
  ...
  it('does not fetch when tenantId is empty string', () => {
    const { result } = renderHook(() => useEntityMetadata(1, ''), {
      wrapper: createWrapper(),
    });
  ...
  it('sets error when fetch fails', async () => {
    ...
    const { result } = renderHook(() => useEntityMetadata(99, 'tenant-x'), {
      wrapper: createWrapper(),
    });
