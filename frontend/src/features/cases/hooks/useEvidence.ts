...
export const evidenceKeys = {
  ...
  statistics: (caseId: string) => [...evidenceKeys.all, 'statistics', caseId] as const,
  auditLog: (evidenceId: string) => [...evidenceKeys.all, 'audit-log', evidenceId] as const,
  search: (filters: EvidenceSearchFilters) => [...evidenceKeys.all, 'search', filters] as const,
};
...
export const useCaseEvidence = (
  ...
  return useQuery({
    ...
    queryFn: () => evidenceService.getCaseEvidence(caseId, filters, page, limit),
 ...
 */
export const useEvidenceDetails = (evidenceId: string, enabled: boolean = true) => {
  return useQuery({
 ...
 */
export const useEvidenceStatistics = (caseId: string, enabled: boolean = true) => {
  return useQuery({
 ...
 */
export const useEvidenceAuditLog = (evidenceId: string, enabled: boolean = true) => {
  return useQuery({
...
export const useUploadEvidence = (caseId: string) => {
  ...
  return useMutation({
    mutationFn: (data: UploadEvidenceDto) => evidenceService.uploadEvidence(data),
    onSuccess: (response) => {
      queryClient.invalidateQueries({ queryKey: evidenceKeys.list(caseId, undefined) });
      queryClient.invalidateQueries({ queryKey: evidenceKeys.statistics(caseId) });
      toast.success(`Evidence "${response.evidence.file_name}" uploaded successfully`);
    },
    onError: (error: Error) => {
...
export const useVerifyEvidence = (caseId: string) => {
  ...
  return useMutation({
    mutationFn: (data: VerifyEvidenceDto) => evidenceService.verifyEvidence(data),
    onSuccess: (response) => {
      queryClient.invalidateQueries({ queryKey: evidenceKeys.detail(response.evidence_id) });
      queryClient.invalidateQueries({ queryKey: evidenceKeys.list(caseId, undefined) });
...
export const useUpdateEvidenceMetadata = (caseId: string) => {
  ...
  return useMutation({
    mutationFn: ({ evidenceId, updates }: { evidenceId: string; updates: Parameters<typeof evidenceService.updateEvidenceMetadata>[1] }) =>
      evidenceService.updateEvidenceMetadata(evidenceId, updates),
    onSuccess: (evidence) => {
      queryClient.invalidateQueries({ queryKey: evidenceKeys.detail(evidence.evidence_id) });
      queryClient.invalidateQueries({ queryKey: evidenceKeys.list(caseId, undefined) });
...
export const useDownloadEvidence = () => {
  return useMutation({
    mutationFn: (evidenceId: string) => evidenceService.downloadEvidence(evidenceId),
    onSuccess: (response) => {
      // Trigger download
