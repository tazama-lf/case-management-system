...
const CaseInformationCard: React.FC<CaseInformationCardProps> = ({
  ...
  const informationItems = [
    ...
    { label: 'Assignment Date', value: caseInformation.assignmentDate },
    {
      ...
      value: (
        <span
          className={`inline-flex rounded-full px-2 py-1 text-xs font-medium ${getStatusColor(caseInformation.status)}`}
        >
    ...
    },
    {
      ...
      value: (
        <span
          className={`text-sm font-medium ${getPriorityColor(caseInformation.priority)}`}
        >
