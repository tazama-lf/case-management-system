...
const WorkQueueTable: React.FC<WorkQueueTableProps> = ({
  ...
  const [loadingAlertForTask, setLoadingAlertForTask] = useState<string | null>(
    null,
  );
  ...
  /**
   * Gets available actions for a task based on its status and user permissions
   */
  const getAvailableActions = (task: UnifiedWorkQueueTask) => {
    const actions: React.ReactNode[] = [];

    // Assignment actions
    if (!task.assignee && onAssign) {
      actions.push(
        <button
          key="assign"
  ...
  React.useEffect(() => {
    if (investigators.length === 0) fetchInvestigatorsList();
    if (supervisors.length === 0) fetchSupervisorsList();
  }, []);
              ...
              <tr key={task.id || `task-${index}`} className="hover:bg-gray-50">
                <td className="px-4 py-3">
                  <div className="flex flex-col">
                    <div
                      className="text-xs font-medium text-gray-900 font-mono break-all"
                      title={task.id || 'No ID'}
                    >
                      {task.id || 'No ID'}
                    </div>
