...
import { dwhService, type CustomerProfileResponse } from '../../services/dwhService';

interface CustomerProfileTabProps {
  transactionId?: string;
}

const CustomerProfileTab: React.FC<CustomerProfileTabProps> = ({ transactionId }) => {
  const [profile, setProfile] = React.useState<CustomerProfileResponse | null>(null);
    ...
    return (
      <div className="flex items-center justify-center py-12">
        <div className="text-center">
          ...
          <p className="mt-3 text-sm text-gray-600">Loading customer profile...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="rounded-lg border border-red-200 bg-red-50 p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-red-800">Failed to load customer profile</p>
              <p className="mt-1 text-sm text-red-700">{error}</p>
              {transactionId && (
                <p className="mt-1 text-xs text-red-600">Transaction ID: {transactionId}</p>
              )}
            </div>
          </div>
        </div>
      ...
      <div className="space-y-4">
        <div className="rounded-lg border border-yellow-200 bg-yellow-50 p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-yellow-800">No transaction ID available</p>
              <p className="mt-1 text-sm text-yellow-700">
                Customer profile data requires a transaction ID. Enter one manually to search.
              </p>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {transactionId && (
          <span className="text-xs text-gray-500">DWH Transaction ID: {transactionId}</span>
        )}
      </div>
        ...
        <section className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-900">{customerDetails?.customerId || '—'}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-gray-900">{customerDetails?.email || '—'}</p>
            </div>
            <div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <section className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
          <h3 className="mb-3 text-sm font-semibold text-gray-900">Sender Account</h3>
          <div className="space-y-3">
            <div>
              <label className="block text-xs font-medium text-gray-500">Account ID</label>
              <p className="mt-1 text-sm text-gray-900">{senderAccount?.id || '—'}</p>
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-500">Account Type</label>
              <p className="mt-1 text-sm text-gray-900 capitalize">{senderAccount?.accountType || '—'}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-gray-900 capitalize">{senderAccount?.riskRating || '—'}</p>
            </div>
          </div>
        </section>
        ...
        <section className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
          <h3 className="mb-3 text-sm font-semibold text-gray-900">Receiver Account</h3>
          <div className="space-y-3">
            <div>
              <label className="block text-xs font-medium text-gray-500">Account ID</label>
              <p className="mt-1 text-sm text-gray-900">{receiverAccount?.id || '—'}</p>
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-500">Account Type</label>
              <p className="mt-1 text-sm text-gray-900 capitalize">{receiverAccount?.accountType || '—'}</p>
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-500">Opened On</label>
              <p className="mt-1 text-sm text-gray-900">
            ...
            <div>
              <label className="block text-xs font-medium text-gray-500">Risk Rating</label>
              <p className="mt-1 text-sm text-gray-900 capitalize">{receiverAccount?.riskRating || '—'}</p>
            </div>
          </div>
        </section>
      </div>
      ...
      <section className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-gray-900">{address?.postalCode || '—'}</p>
          </div>
          <div>
