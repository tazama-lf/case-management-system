...
const ReferenceResultsTable: React.FC<ReferenceResultTableProps> = ({
  ...
  return (
    <div className="bg-white shadow-sm rounded-lg border border-gray-200 overflow-hidden">
            ...
            {data.map((data) => (
              <tr key={data.id.toString()} className="hover:bg-gray-50">
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="text-sm font-medium text-gray-900">{data.id.toString()}</div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="text-sm font-medium text-gray-900">{data.txTp}</div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
      ...
      {pagination && <TablePagination pagination={pagination} itemLabel="work queues" />}
    </div>
  );
};
