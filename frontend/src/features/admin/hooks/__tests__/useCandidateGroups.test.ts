...
describe('useCandidateGroups', () => {
    ...
    it('respects custom pageSize param', async () => {
        ...
        await waitFor(() =>
            expect(mockGetCandidateGroups).toHaveBeenCalledWith({ size: 25, start: 0 }),
        );
    });
    ...
    it('refetches when onPageChange is called', async () => {
        mockGetCandidateGroups
            ...
            .mockResolvedValueOnce({ items: [{ id: '2', name: 'Q2', type: 'AML' }], totalCount: 20 });
        ...
        await waitFor(() =>
            expect(mockGetCandidateGroups).toHaveBeenCalledWith({ size: 10, start: 10 }),
        );
    });
