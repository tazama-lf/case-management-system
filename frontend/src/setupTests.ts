...
globalThis.window ??= {};
globalThis.window.location ??= { href: '', assign: () => {}, replace: () => {} };
// Ensure AbortController/AbortSignal and Request come from the same implementation
if (typeof globalThis.AbortController === 'undefined' && (globalThis as any).window?.AbortController) {
...
if (typeof globalThis.AbortSignal === 'undefined') {
	...
	globalThis.AbortSignal = (globalThis as any).window?.AbortSignal ?? globalThis.AbortController?.prototype?.signal?.constructor;
}
...
const _OriginalRequest = (globalThis as any).Request ?? (globalThis as any).fetch?.Request;
if (_OriginalRequest) {
...
const ignoreUnhandled = (reason: any) => {
	...
	if (
		typeof msg === 'string' &&
		(msg.includes('Expected signal ("AbortSignal') || msg.includes('window is not defined'))
	) {
		return true;
	}
	return false;
