...
export class FlowableEventListener {
  ...
  async handleCaseCreated(event: CaseCreatedEvent) {
    try {
      ...
      this.logger.log(`Started Flowable process ${processInstance.id} for case ${event.caseId}`, FlowableEventListener.name);
    } catch (error) {
      this.logger.error(
    ...
    try {
      ...
      let flowableTask = flowableTasks.find((t: any) => {
        ...
        const isMatchingTask =
          t.taskDefinitionKey === this.getTaskDefinitionKey(event.taskName) || (t.name === event.taskName && !hasPostgresId);
      ...
      if (flowableTask) {
        ...
        if (shouldCreateNewTask) {
          ...
          this.logger.log(`Created new Flowable task ${flowableTask.id} for PostgreSQL task ${event.taskId}`, FlowableEventListener.name);
        } else {
          this.logger.warn(`BPMN task not yet created for ${event.taskName}, will be synced when available`, FlowableEventListener.name);
        }
      }
    } catch (error) {
  ...
  private isBpmnDefinedTask(taskName: string): boolean {
    const bpmnTasks = ['Approve Case Creation', 'Investigate Case', 'Investigate case', 'Approve case closure'];
  ...
  async handleCaseSuspended(event: CaseSuspendedEvent) {
    try {
      const processInstance = await this.flowableService.getProcessInstanceByBusinessKey(event.caseId);
      ...
      if (processInstance) {
        ...
        this.logger.log(`Suspended Flowable process for case ${event.caseId}`, FlowableEventListener.name);
      }
    } catch (error) {
      this.logger.error(`Failed to suspend Flowable process: ${error.message}`, error.stack, FlowableEventListener.name);
    }
  }
  @OnEvent('case.resumed')
  async handleCaseResumed(event: CaseResumedEvent) {
    try {
      const processInstance = await this.flowableService.getProcessInstanceByBusinessKey(event.caseId);

      if (processInstance) {
        await this.flowableService.activateProcessInstance(processInstance.id);

        this.logger.log(`Resumed Flowable process for case ${event.caseId}`, FlowableEventListener.name);
      }
    } catch (error) {
