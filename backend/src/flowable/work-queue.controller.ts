...
export class WorkQueueController {
  ...
  async getMyTasks(@Req() req: AuthenticatedRequest, @Query('includeCompleted') includeCompleted?: string) {
    try {
      const userId = req.user.token.clientId;
  ...
  async claimTask(@Param('taskId') taskId: string, @Req() req: AuthenticatedRequest) {
    try {
      const userId = req.user.token.clientId;
  ...
  async releaseTask(@Param('taskId') taskId: string, @Req() req: AuthenticatedRequest) {
    try {
      const userId = req.user.token.clientId;
  ...
  async delegateTask(@Param('taskId') taskId: string, @Body('userId') delegateToUserId: string, @Req() req: AuthenticatedRequest) {
    try {
      ...
      const task = await this.flowableService.getTask(taskId);
      if (task?.variables?.postgres_task_id) {
        await this.taskService.assignTaskToInvestigator(task.variables.postgres_task_id, delegateToUserId, supervisorId, tenantId);
      }
