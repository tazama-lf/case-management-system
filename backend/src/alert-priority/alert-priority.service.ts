...
export class AlertPriorityService implements OnModuleInit {
  ...
  onModuleInit() {
    ...
    this.logger.log(`Alert priority recalculation scheduled every ${this.updateIntervalMs / 1000} seconds.`);
  }
  ...
  async runRecalculation() {
    ...
    for (const alert of alerts) {
      try {
        ...
        const [urgentThreshold, criticalThreshold, breachThreshold] = this.urgencyThresholds;
        let urgency = 'New';
        ...
        this.logger.debug(`Alert ${alert.alert_id}: priority=${priorityScore} urgency=${urgency}`);
      } catch (err) {
