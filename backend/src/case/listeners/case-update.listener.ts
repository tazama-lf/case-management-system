...
export class CaseUpdateListener {
  ...
  async handleCaseUpdateRequested(event: { caseId: string; updateData: any; userId: string }) {
    try {
      ...
      this.logger.error(`Failed to update case ${event.caseId}: ${error.message}`, error.stack, CaseUpdateListener.name);
    }
  }
}
