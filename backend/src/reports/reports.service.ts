...
export class ReportsService {
  ...
  async getInvestigatorWorkload(dateRange?: string) {
    ...
    // Generate efficiency data (resolution times by investigator)
    ...
    // Generate outcome data (case outcomes by investigator)
    ...
    // Generate performance data for the table
    const performanceData = await Promise.all(
      investigators.map(async ({ case_owner_user_id }) => {
        if (!case_owner_user_id) return null;
        
        const [totalCases, activeCases, closedCases, pendingTasks] = await Promise.all([
          this.prisma.case.count({
            where: {
              case_owner_user_id,
              created_at: { gte: startDate, lte: endDate },
        ...
        return {
          ...
          totalCases,
          activeCases,
          completedCases: closedCases, // Use closedCases as completedCases
          pendingTasks,
          completionRate,
          avgResolutionTime: 0, // Will be calculated from efficiency data
          caseClosureRate: completionRate, // Use completion rate as closure rate
