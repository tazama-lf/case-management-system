...
export class CaseCreationApprovalService {
    ...
    async manualCaseCreate(dto: ManualCreateCaseDto, userId: string, tenantId: string, role: string) {
        ...
        try {
            ...
            this.logger.log(`[ManualCase] Manual case creation completed successfully for case ${result.case.case_id}`, CaseCreationApprovalService.name);
    ...
    async approveCaseCreation(caseId: string, supervisorId: string, tenantId: string) {
        try {
            this.logger.log(`[ApproveCaseCreation] Supervisor ${supervisorId} approving case creation for case ${caseId}`, CaseCreationApprovalService.name);
            ...
            const approvalTask = await this.caseRepository.findTaskByNameAndStatus(caseId, 'Approve Case Creation', TaskStatus.STATUS_01_UNASSIGNED);
            ...
            setTimeout(async () => {
                try {
                    const investigationTask = await this.caseRepository.findTaskByNames(caseId, [...TASK_NAMES.INVESTIGATE_CASE_VARIANTS], TaskStatus.STATUS_01_UNASSIGNED);
                    ...
                    this.logger.warn(`[ApproveCaseCreation] Failed to verify investigation task creation: ${error.message}`, CaseCreationApprovalService.name);
                }
            }, 3000);
            ...
            this.logger.error(`[ApproveCaseCreation] Failed to approve case creation: ${error.message}`, error.stack, CaseCreationApprovalService.name);
    ...
    async rejectCaseCreation(caseId: string, supervisorId: string, tenantId: string, reason: string) {
        try {
            ...
            if (!reason || reason.trim().length < VALIDATION_LENGTHS.MIN_REOPENING_REASON) {
                throw new BadRequestException(`Rejection reason is required and must be at least ${VALIDATION_LENGTHS.MIN_REOPENING_REASON} characters`);
            }
            ...
            this.logger.error(`Failed to reject case creation for case ${caseId}: ${error.message}`, error.stack, CaseCreationApprovalService.name);
            await this.auditLogService.logAction({
                userId: supervisorId,
                operation: 'rejectCaseCreation',
                entityName: CaseCreationApprovalService.name,
