...
export class CaseClosureApprovalService {
            ...
            if (role === 'CMS_SUPERVISOR') {
                ...
                await this.flowableService.handleTaskCompleted({
                    ...
                    completionVariables: {
                        ...
                        recommededOutcome: dto.recommendedOutcome,
    ...
    async approveCaseClosure(caseId: string, finalOutcome: string, comments: string, supervisorId: string) {
        try {
            ...
            const approvalTask = caseDetails.tasks.find((t) => t.name && TASK_NAMES.APPROVE_CASE_CLOSURE_VARIANTS.includes(t.name as any) && t.status === TaskStatus.STATUS_01_UNASSIGNED);
