...
export class CaseClosureApprovalService {
    ...
    async closeCase(caseId: string, dto: CloseCaseDto, userId: string, tenantId: string, role: string) {
        try {
            ...
            const investigationTask = caseData.tasks.find(
                (task) => TASK_NAMES.INVESTIGATE_CASE_VARIANTS.includes(task.name as any),
            );
            ...
            this.logger.log(`[CloseCase] Investigation task ${investigationTask.task_id} current status: ${oldTaskStatus}`, CaseClosureApprovalService.name);
    ...
    async approveCaseClosure(caseId: string, finalOutcome: string, comments: string | undefined, supervisorId: string) {
        try {
            this.logger.log(`[ApproveCaseClosure] Supervisor ${supervisorId} attempting to approve case closure for ${caseId}`, CaseClosureApprovalService.name);
            ...
            const approvalTask = caseDetails.tasks.find(
                (t) =>
                    t.name && TASK_NAMES.APPROVE_CASE_CLOSURE_VARIANTS.includes(t.name as any) &&
            ...
            this.logger.log(`[ApproveCaseClosure] Case ${caseId} closure approved successfully with outcome ${finalOutcome}`, CaseClosureApprovalService.name);
