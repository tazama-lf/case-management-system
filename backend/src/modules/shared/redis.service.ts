...
export class RedisService implements OnModuleInit, OnModuleDestroy {
     ...
     */
    async get<T = string>(key: string, parseJson: boolean = false): Promise<T | null> {
        if (!this.isConnected()) {
     ...
     */
    async mget<T = string>(keys: string[], parseJson: boolean = false): Promise<Record<string, T | null>> {
        if (!this.isConnected()) {
