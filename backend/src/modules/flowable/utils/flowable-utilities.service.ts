...
export class FlowableUtilitiesService {
  ...
  async findUnmappedBpmnTask(
    ...
    for (const task of flowableTasks) {
      ...
      if (candidateGroup) {
        ...
        const matchesGroup = taskCandidateGroups.some(
          (g: string) => g.toLowerCase() === candidateGroup.toLowerCase(),
        );
        if (matchesGroup) {
          return task;
        }
  ...
  async getTaskVariables(taskId: string): Promise<Record<string, string>> {
    try {
      ...
      this.logger.error(
        `Failed to get task variables: ${error.message}`,
        error.stack,
        FlowableUtilitiesService.name,
      );
      return {};
    }
  }
