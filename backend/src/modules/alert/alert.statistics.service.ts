...
export class AlertStatisticsService {
  ...
  async getAlertDetails(alertId: number, tenantId: string, userId: string) {
    try {
      ...
      this.logger.log(`Alert ${alertId} opened by user ${userId} for review at ${new Date().toISOString()}`, AlertStatisticsService.name);
      ...
      this.logger.error(`Failed to fetch alert ${alertId}: ${error.message}`, AlertStatisticsService.name);
  ...
  async getAlertActionHistory(alertId: number, tenantId: string, userId: string) {
    ...
    // TODO: Should be eventLogService instead of auditLogService
