...
import { AlertResponseDto, AlertActionHistoryDTO, AlertDetailsResponseDTO } from './dto';
...
export class AlertController {
  ...
  async getAlertActionHistory(@Param('alertId') alertId: number, @Req() req: AuthenticatedRequest) {
    ...
    return this.alertStatisticsService.getAlertActionHistory(alertId, tenantId, userId);
  }
  ...
  async getAlertDetails(@Param('alertId') alertId: number, @Req() req: AuthenticatedRequest) {
    ...
    if (!tenantId) throw new BadRequestException('Missing tenantId');
    if (!userId) throw new BadRequestException('Missing userId');
    return this.alertStatisticsService.getAlertDetails(alertId, tenantId, userId);
  }
}
