...
export class TaskLifecycleService {
	...
	async assignTaskToInvestigator(taskId: string, assignedUserId: string, supervisorId: string, tenantId: string) {
		...
		this.emitCaseStatusChange(existingTask.case_id, previousCaseStatus, CaseStatus.STATUS_10_ASSIGNED, `Case assigned to investigator ${assignedUserId} by supervisor ${supervisorId}`);
	...
	async reassignTask(taskId: string, actorUserId: string, tenantId: string, assignedUserId: string) {
		...
		this.emitAssignment(taskId, result.updatedTask.case_id, assignedUserId, previousAssignedUserId || undefined);
		this.emitCaseStatusChange(existingTask.case_id, previousCaseStatus, CaseStatus.STATUS_10_ASSIGNED, `Case reassigned to investigator ${assignedUserId} by ${actorUserId}`);
	...
	async selfAssignTask(taskId: string, investigatorUserId: string, tenantId: string) {
		...
		if (existingTask.status !== TaskStatus.STATUS_01_UNASSIGNED) throw new BadRequestException(`Task ${taskId} must be unassigned to self-assign.`);
		const existingCase = await this.getCaseOrThrow(existingTask.case_id);
		const previousCaseStatus = existingCase.status;
		...
		this.emitCaseStatusChange(existingTask.case_id, previousCaseStatus, CaseStatus.STATUS_10_ASSIGNED, `Case self-assigned by investigator ${investigatorUserId}`);
	...
	async unassignTask(taskId: string, actorUserId: string, tenantId: string, reason: string) {
		...
		const existingTask = await this.getTaskOrThrow(taskId);
		if (existingTask.status === TaskStatus.STATUS_30_COMPLETED) throw new BadRequestException(`Cannot unassign a completed task (${taskId})`);
		...
		this.eventEmitter.emit('task.unassigned', new TaskUnassignedEvent(taskId, result.updatedTask.case_id, existingTask.assigned_user_id || undefined, candidateGroup, reason));
		this.emitCaseStatusChange(existingTask.case_id, previousCaseStatus, CaseStatus.STATUS_02_READY_FOR_ASSIGNMENT, `Task unassigned. Reason: ${reason}`);
		...
		return { ...result.updatedTask, message: `Task successfully unassigned and returned to ${candidateGroup} work queue`, candidateGroup, unassignmentReason: reason };
	}
	...
	async releaseTask(taskId: string, actorUserId: string) {
		...
		this.eventEmitter.emit('task.unassigned', new TaskUnassignedEvent(taskId, updatedTask.case_id, previousAssignedUserId || undefined, existingTask.candidateGroup || undefined));
		await this.auditLogService.logAction({
			userId: actorUserId,
	...
	async completeTask(taskId: string, actorUserId: string) {
		...
		this.eventEmitter.emit('task.status.changed', new TaskStatusChangedEvent(taskId, updatedTask.case_id, updatedTask.name || '', existingTask.status, TaskStatus.STATUS_30_COMPLETED, updatedTask.assigned_user_id || undefined));
		await this.auditLogService.logAction({
			userId: actorUserId,
	...
	async reassignTaskToWorkQueue(taskId: string, targetWorkQueueId: string, userId: string, tenantId: string, reason?: string, assignedUserId?: string) {
		return this.prisma.$transaction(async (tx) => {
			const task = await tx.task.findUnique({
				where: { task_id: taskId },
				include: { workQueue: { select: { work_queue_id: true, name: true } }, case: { select: { case_id: true, tenant_id: true, status: true } } },
			});
			if (!task) throw new NotFoundException(`Task ${taskId} not found`);
			if (task.case.tenant_id !== tenantId) throw new ForbiddenException('Task does not belong to your organization');
			if (task.status === TaskStatus.STATUS_20_IN_PROGRESS && task.assigned_user_id) throw new BadRequestException('Cannot reassign in-progress task');
			...
			if (assignedUserId) {
				...
				if (!memberAssignment) throw new BadRequestException(`User ${assignedUserId} is not assigned to target work queue '${targetQueue.name}'`);
			}
			...
			const auditDescription = reason ? `Reassigned task from '${oldWorkQueueName || 'unassigned'}' to '${targetQueue.name}'. Reason: ${reason}` : `Reassigned task from '${oldWorkQueueName || 'unassigned'}' to '${targetQueue.name}'`;
			await this.auditLogService.logAction({
				userId,
	...
	private emitCaseStatusChange(caseId: string, prev: CaseStatus, next: CaseStatus, reason: string) {
		this.eventEmitter.emit('case.status.changed', new CaseStatusChangedEvent(caseId, prev, next, reason));
	}
