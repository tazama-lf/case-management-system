...
import { Prisma, TaskStatus, CaseStatus } from '@prisma/client';
...
export class TaskRepository {
	...
	async assignTaskAndUpdateCase(taskId: string, assignedUserId: string) {
		return this.transaction(async (tx) => {
			...
			await this.updateCase(task.case_id, { status: CaseStatus.STATUS_10_ASSIGNED, case_owner_user_id: assignedUserId, updated_at: new Date() }, tx);
	...
	async unassignTaskAndUpdateCase(taskId: string) {
		return this.transaction(async (tx) => {
			...
			await this.updateCase(task.case_id, { status: CaseStatus.STATUS_02_READY_FOR_ASSIGNMENT, case_owner_user_id: null, updated_at: new Date() }, tx);
			return { taskBefore: task, updatedTask, previousCaseStatus: caseRecord.status };
		});
	}
