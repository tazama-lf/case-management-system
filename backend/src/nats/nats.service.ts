...
export class NatsStartupService implements OnModuleInit {
  ...
  async handleMessage(req: AlertMessageDto) {
    ...
    try {
      ...
      const alert = await this.triageService.handleNewAlert(submitAlertDto, systemId, tenantId, 'NATS');
      const aiTriageEnabled = this.configService.get<string>('AI_TRIAGE_ENABLED', 'false').toLowerCase() === 'true';
      if (aiTriageEnabled) {
        this.logger.log(`AI Triage enabled doing ai triage on: ${alert.alert_id}`, NatsStartupService.name);
        await this.triageService.handleAITriage(alert.alert_id, alert.case_id, submitAlertDto, systemId, tenantId);
      } else {
        this.logger.log(`AI Triage disabled creating manual investiation task for alert: ${alert.alert_id}`, NatsStartupService.name);
