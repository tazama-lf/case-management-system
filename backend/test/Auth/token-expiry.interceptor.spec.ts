...
describe('TokenExpiryInterceptor', () => {
  ...
  it('should call next.handle() if token is valid', () => {
    ...
    expect(interceptor.intercept(context as ExecutionContext, callHandler as CallHandler)).toBeInstanceOf(Object);
  });

  it('should throw UnauthorizedException if token is expired', () => {
    (authService.isTokenExpired as jest.Mock).mockReturnValue(true);
    expect(() => interceptor.intercept(context as ExecutionContext, callHandler as CallHandler)).toThrow(UnauthorizedException);
  });
  ...
  it('should throw UnauthorizedException in catchError if error is 401 and token is expired', (done) => {
    ...
    callHandler.handle = jest.fn().mockReturnValue(throwError(() => ({ status: 401 })));
  ...
  it('should rethrow error in catchError if not 401', (done) => {
    (authService.isTokenExpired as jest.Mock).mockReturnValue(false);
    callHandler.handle = jest.fn().mockReturnValue(throwError(() => ({ status: 500 })));
    interceptor.intercept(context as ExecutionContext, callHandler as CallHandler).subscribe({
      error: (err) => {
